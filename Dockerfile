...
# linux-headers build-base python2 python2-dev py-setuptools \
...
#RUN chown databox:databox /root/go
# USER databox
WORKDIR /root/go

ENV GOPATH="/root/go"
RUN go get -u github.com/golang/dep/cmd/dep
RUN mkdir -p /root/go/src/main/my-app
WORKDIR /root/go/src/main

# angular build deps (-g ??)
RUN cd /root/go/src/main/my-app && npm install @angular/cli
...
RUN cd /root/go/src/main/my-app && `npm bin`/ng build -bh /databox-app-activity-summary/ui/static/ && cp dist/* ../www/
